...
class DataObject implements \ArrayAccess
{
    ...
    public function getData($key = '', $index = null)
    {
        ...
        if ($index !== null) {
            if ($data === (array)$data) {
                ...
                $data = isset($data[$index]) ? $data[$index] : null;
            } elseif ($data instanceof \App\Api\Data\DataObject) {
    ...
    public function toArray(array $keys = [])
    {
        if (empty($keys)) {
            ...
            if (count($currentKeys)) {
                foreach ($data as $key => $value) {
                    if ($value instanceof \App\Api\Data\DataObject) {
                        $data[$key] = $value->toArray();
                        // https://www.php.net/manual/en/function.array-is-list.php
                    }else if (is_array($value) && array_is_list($value)) {
                        $_currents = [];
                        foreach ($value as $val) {
                            if ($val instanceof \App\Api\Data\DataObject) {
                                $_currents[] = $val->toArray();
                            }else{
                                $_currents[] = $val;
                            }
                        }
    ...
    public function debug($data = null, &$objects = [])
    {
        ...
        foreach ($data as $key => $value) {
            ...
            } elseif ($value instanceof \App\Api\Data\DataObject) {

...
<body>
    ...
    <div class='examples'>
        ...
        <div class='parent'>
            ...
            <div class='wrapper'>
                <div id='left-events' class='container'>
                    ...
                    <div>Whenever an element is cloned because <code>copy: true</code>, a <code>cloned</code> event
                        fires
                    </div>
                    <div>The <code>shadow</code> event fires whenever the placeholder showing where an element would be
                        dropped is moved to a different container or position</div>
                    <div>A <code>drop</code> event is fired whenever an element is dropped anywhere other than its
                        origin
                ...
                <div id='right-events' class='container'>
                    ...
                    <div>A <code>cancel</code> event is fired when an element would be dropped onto an invalid target,
                        but
                        retains its original placement instead</div>
                    <div>The <code>over</code> event fires when you drag something over a container, and
                        <code>out</code>
                        fires when you drag it away from the container
                    </div>
                    <div>Lastly, a <code>dragend</code> event is fired whenever a drag operation ends, regardless of
                        whether
                        it ends in a cancellation, removal, or drop</div>
                </div>
            </div>
            <pre>
                <code>
        ...
        <div class='parent'>
            <label for='hy'>Need to be able to quickly delete stuff when it spills out of the chosen containers? Note
                how you can easily sort the items in any containers by just dragging and dropping.</label>
            <div class='wrapper'>
        ...
        <div class='parent'>
            <label for='hy'>By default, dropping an element outside of any known containers will keep the element in
                the last place it went over. You can make elements go back to origin if they're dropped outside of known
                containers, too.</label>
            <div class='wrapper'>
                <div id='left-rollbacks' class='container'>
                    <div>Moving items between containers works as usual</div>
                    <div>If you try to drop an item outside of any containers, though, it'll retain its original
                        position
                    </div>
                    <div>When that happens, a <code>cancel</code> event will be raised</div>
                </div>
                <div id='right-rollbacks' class='container'>
                    <div>Note that the dragged element will go back to the place you originally dragged it from, even if
                        you
                        move it over other containers</div>
                    <div>This is useful if you want to ensure drop events only happen when the user intends for them to
                        happen explicitly, avoiding surprises</div>
                </div>
            </div>
            <pre>
                <code>
        ...
        <script>
            dragula([document.getElementById('left-rollbacks'), document.getElementById('right-rollbacks')], {
                revertOnSpill: true
            });
        </script>
        {{-- =========================================================================================== --}}
        ...
        <div class='parent'>
            ...
            <div class='wrapper'>
                <div id='left-copy' class='container'>
                    ...
                    <div>Copying prevents original elements from being dragged. A copy gets created and <em>that</em>
                        gets
                        dragged instead</div>
                ...
                <div id='right-copy' class='container'>
                    ...
                    <div>You'll be dragging a copy, so when they're dropped into another container you'll see the
                        duplication.</div>
                </div>
            </div>
            <pre>
        ...
        <div class='parent'>
            <label for='hy'>Copying stuff from only one of the containers and sorting on the other one? No
                problem!</label>
            <div class='wrapper'>
        ...
        <div class='parent'>
            ...
            <div class='wrapper'>
                <div id='left-lovehandles' class='container'>
                    ...
                    <div><span class='handle'>+</span>Note that <code>handle</code> element in the <code>moves</code>
                        handler is just the original event target.</div>
                </div>
                <div id='right-lovehandles' class='container'>
                    <div><span class='handle'>+</span>This might also be useful if you want multiple children of an
                        element
                        to be able to trigger a drag event.</div>
                    <div><span class='handle'>+</span>You can also use the <code>moves</code> option to determine
                        whether an
            ...
            <div>There are a few similar mechanisms to determine whether an element can be dragged from a certain
                container
                <a href='https://github.com/bevacqua/dragula#optionsmoves'>(<code>moves</code>)</a>, whether an element
                can
                be dropped into a certain container at a certain position <a
                    href='https://github.com/bevacqua/dragula#optionsaccepts'>(<code>accepts</code>)</a>, and whether an
                element is able to originate a drag event <a
                    href='https://github.com/bevacqua/dragula#optionsinvalid'>(<code>invalid</code>)</a>.
            </div>
        </div>
    ...
    <h3 class='promo'>Who couldn't love a pun that good? &mdash; <a
            href='http://thenextweb.com/dd/2015/07/20/less-of-a-drag-maaaaaaaan'>The Next Web</a></h3>
